...
struct Library
{
	...
	void init()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			in.getline(publisher, 100, '-');
			in.getline(genre, 100);

			books[i].title = new char[strlen(title) + 1];
			strcpy(books[i].title, title);
			books[i].author = new char[strlen(author) + 1];
			strcpy(books[i].author, author);
			books[i].publisher = new char[strlen(publisher) + 1];
	...
	{
		printAll();
		cout << "Which book you want to delete? (number): ";
		int book;
		cin >> book; cin.ignore(); book -= 1;
		delValueArray(books, size, book);
		SetColor(Green, White);
	...
	void saveBooks()
	{
		...
		for (int i = 0; i < size; i++)
		{
			out << books[i].title << "-";
			out << books[i].author << "-";
			out << books[i].publisher << "-";
			out << books[i].genre << endl;
		}
		out.close();
		exit(0);
	}
